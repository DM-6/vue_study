...
export default {
    ...
    async beforeMount(){
        if(this.$route.query.geohash){
            ...
            // this.geohash = this.$route.params.geohash    params 与 query的区别
        } else{
    ...
    async mounted(){
        ...
        console.log(data);
        let resLength = data.length;    // 得到数组总长度
        let resArr = [...data];      // ...data 数组展开  一维数组
        let foodArr = [];      // 准备一个空数组 二维
        for(let i = 0, j = 0; i < resLength; i += 8, j++){   // i j 两个计数器
            foodArr[j] = resArr.splice(0, 8);
        }
        this.foodTypes = foodArr;    // 将其交给foodTypes
        this.$nextTick(() => {         // 回调函数
        ...
        // vnode => dom diff 差
        // MVVM 数据改变 到 界面更新 是异步操作 vnode  dom node
        // vue 等待这个过程结束（数据 界面更新完了）  => $nextTick()
...
<style lang="stylus" scoped>    // scoped  样式不会影响外界
@import "../style/mixin"
